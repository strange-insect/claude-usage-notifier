...
Claude Code がログイン時に保存する OAuth トークン（`%APPDATA%\Claude\.credentials.json` 等）を読み取り、Anthropic の内部エンドポイント `api.anthropic.com/api/oauth/usage` を 5 分ごとに叩いて使用率を取得します。**API キー不要**、Claude Code にサブスクでログイン済みなら追加設定は要りません。
...
トレイ（メニューバー）にアイコンが表示され、起動直後に初回ポーリングが走り、以降は 5 分ごとにポーリングします。
...
5 分ポーリングなので 1 日あたり約 288 行（~16 KB）。Excel / pandas でそのまま読めます。肥大化が気になれば手動で退避してください（ローテーション無し）。
